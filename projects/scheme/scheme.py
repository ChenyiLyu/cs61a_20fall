...
def eval_all(expressions, env):
    ...
    while temp.rest is not nil:
        ...
        temp = temp.rest
    return scheme_eval(temp.first, env) # replace this with lines of your own code
    # END PROBLEM 7
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(symbol, scheme_eval(expressions.rest.first,env))
        return symbol
        # END PROBLEM 5
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 9
        symbol, formal, body = target.first, target.rest, expressions.rest
...
def do_quote_form(expressions, env):
    ...
    # BEGIN PROBLEM 6
    return expressions.first # question mark: why not expression.rest?
    # END PROBLEM 6
