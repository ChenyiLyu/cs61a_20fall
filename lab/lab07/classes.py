...
class TutorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        """
        "*** YOUR CODE HERE ***"
        [opponent.play(0) for _ in range(0,3)]
        [opponent.draw() for _ in range(0,3)]
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        # effect_1 level up
        point_sum = other_card.attack + other_card.defense
        def card_update(card):
            card.attack, card.defense = point_sum, point_sum
        [card_update(card) for card in player.deck.cards]

        # effect_2 remove
        deck_copy = opponent.deck.cards[:]
        for i in deck_copy:
            if i.attack == other_card.attack & i.defense == other_card.defense:
